...
Your job is to help students learn effectively through:
...
function checkFeasibility(stages, dailyMinutes, durationDays) {
  const totalMinutesNeeded = stages.reduce(
    (total, stage) =>
      total + Math.max(1, Number(stage?.estimatedMinutes) || 0),
    0
  );
  ...
  const minDaysNeeded = Math.max(
    1,
    Math.ceil(totalMinutesNeeded / Number(dailyMinutes))
  );
...
async function generateStudyPlan(state) {
  ...
  const effectiveDuration = feasibility.feasible
    ? durationDays
  ...
  for (const stage of roadmap.stages) {
    ...
    const daysForStage = Math.max(
      1,
      Math.ceil(estimatedMinutes / dailyMinutes)
    );
  ...
  let note = `Study at the ${level} level for ${dailyMinutes} minutes each day and complete the stages in order.`;

  try {
    const aiNote = await hf.generateJSON(
      SYSTEM_PROMPT,
      `
Student level: ${level}
...
Write one short practical study-planning note.
...
Do not mention completed progress.
...
`,
    ...
    if (aiNote && cleanString(aiNote.note)) {
      note = aiNote.note.trim();
    }
  } catch {
    // Keep the safe local note if AI note generation fails.
  }

  if (!feasibility.feasible) {
    note =
      `The roadmap needs about ${feasibility.minDaysNeeded} days at ${dailyMinutes} minutes per day. ${note}`;
  }

  return {
    feasible: feasibility.feasible,
    requestedDurationDays: durationDays,
    effectiveDurationDays: effectiveDuration,
...
async function coachReply(message, context = {}) {
  ...
  const strengths = Array.isArray(
    ...
    ? safeContext.strengths.slice(0, 8)
    : [];

  const weaknesses = Array.isArray(
    safeContext.weaknesses
  )
    ? safeContext.weaknesses.slice(0, 8)
    : [];
...
Strengths:
${strengths.length ? strengths.join(", ") : "None recorded"}

Weaknesses:
${weaknesses.length ? weaknesses.join(", ") : "None recorded"}
