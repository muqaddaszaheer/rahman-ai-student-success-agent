...
app.post("/api/agent/study-plan", asyncRoute(async (req, res) => {
  ...
  if (!body.roadmap || !Array.isArray(body.roadmap.stages) || body.roadmap.stages.length === 0) {
    return res.status(400).json({ error: "A valid roadmap is required." });
  }
  if (!Number.isFinite(dailyMinutes) || dailyMinutes <= 0 ||
      !Number.isFinite(durationDays) || durationDays <= 0) {
    return res.status(400).json({ error: "Daily minutes and duration days must be positive numbers." });
  }
  if (!body.level) {
    return res.status(400).json({ error: "Student level is required." });
  }

  res.json(await agent.generateStudyPlan({
...
app.post("/api/agent/practice", asyncRoute(async (req, res) => {
  ...
  if (!body.topic || typeof body.topic !== "string") {
    return res.status(400).json({ error: "A practice topic is required." });
  }
  if (!body.level) {
    return res.status(400).json({ error: "Student level is required." });
  }

  res.json(await agent.generatePractice({
    topic: body.topic.trim(),
    level: String(body.level),
    currentStageTitle: body.currentStageTitle || "",
    recentWeakAreas: Array.isArray(body.recentWeakAreas) ? body.recentWeakAreas.slice(0, 8) : []
  }));
}));
...
app.post("/api/agent/assessment", asyncRoute(async (req, res) => {
  const body = req.body || {};
  if (!body.topic || typeof body.topic !== "string") {
    return res.status(400).json({ error: "An assessment topic is required." });
  }
  if (!body.level) {
    return res.status(400).json({ error: "Student level is required." });
  }

  res.json(await agent.generateAssessment({
    topic: body.topic.trim(),
    level: String(body.level),
...
app.post("/api/agent/coach", asyncRoute(async (req, res) => {
  ...
  const reply = await agent.coachReply(body.message.trim(), context);
  res.json({ reply });
}));
...
app.listen(PORT, () => {
  console.log(`Rahman AI Student Success Agent running at http://localhost:${PORT}`);
  console.log(`Hugging Face model: ${hf.HF_MODEL}`);
  if (!process.env.HF_TOKEN) {
    console.warn("HF_TOKEN is not set. Copy .env.example to .env and add your token.");
  }
});
